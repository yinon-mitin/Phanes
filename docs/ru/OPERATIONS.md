...
## Очередь Manual Interaction в Sonarr/Radarr

Постоянные уведомления `Manual Interaction` могут появляться, когда завершённые
торренты остаются в активной категории qBittorrent `sonarr` или `radarr`, хотя
файл уже присутствует в медиатеке. Приложения продолжают опрашивать эти
категории и снова показывают старую загрузку как `importPending` или
`importBlocked`.
...
Reconciler работает fail-closed: завершённый warning Radarr перемещается только
если API подтверждает `hasFile=true` у соответствующего фильма. Торренты и их
данные не удаляются. Применение проверенного исправления:

```sh
make arr-queue-reconcile
```

Успешное исправление production настроило `sonarr-imported` и
`radarr-imported` как post-import категории qBittorrent. 25 старых торрентов
Radarr, фильмы которых уже были в медиатеке, перемещены в `radarr-imported`.
Два элемента Sonarr, не являвшиеся импортируемыми эпизодами, сохранены в
`manual-review`. После этого обе активные очереди показали ноль предупреждений
Manual Interaction. Новые успешно импортированные загрузки автоматически
покидают активную категорию и могут продолжать раздачу из прежнего save path.

## SDR-first политика медиаконтента

Текущий телевизор оптимизирован для SDR-воспроизведения. Автоматическая загрузка
использует `RU 2160p SDR` с fallback на `RU 1080p SDR`; существующие назначения
`RU 2160p SDR` и `RU 2160p SDR Fallback 1080p SDR` также совместимы с SDR.
HDR-профили сохранены
как future-ready для возможной замены телевизора и проверяются отдельно.

Каноническая политика находится в `stack/media-policy.json`. Проверка:
